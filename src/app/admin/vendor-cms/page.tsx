...
export default function VendorCMSAdminPage() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    async function fetchCMS() {
      ...
      try {
        const docSnap = await getDoc(doc(db, "vendor_settings", user.uid));
        if (docSnap.exists()) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await setDoc(doc(db, "vendor_settings", user.uid), { cms_storefront: layout }, { merge: true });
      alert("Storefront Layout Saved Successfully! Your vendor profile will immediately reflect these changes if dynamic mode is enabled.");
    } catch (e) {
      console.error(e);
      alert("Error saving Storefront Layout");
    }
    setSaving(false);
  };
  ...
  if (loading) return <div className="text-[#C5A059] animate-pulse">Loading Storefront Data...</div>;

  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl sm:text-3xl font-serif font-bold text-[#C5A059]">Vendor Storefront Builder</h1>
        <p className="text-gray-300 text-xs mt-1">Control the layout, banners, and featured products on your public vendor profile.</p>
      </div>

      {/* Master Toggle */}
      <div className="bg-[#0B2B26] border border-[#C5A059]/30 rounded-2xl p-6 flex justify-between items-center shadow-xl">
        <div>
          <h2 className="text-lg font-bold text-white mb-1">Dynamic Storefront System</h2>
          <p className="text-xs text-gray-400">If enabled, your public storefront will use the configuration below.</p>
        </div>
        <button 
