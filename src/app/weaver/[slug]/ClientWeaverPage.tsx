...
export default function ClientWeaverPage() {
  ...
  if (!weaver || weaver.status === "pending_approval" || weaver.status === "pending" || weaver.status === "rejected") {
    return (
      <div className="flex-1 min-h-screen flex items-center justify-center bg-[#051815] p-6">
        <div className="text-center py-16 bg-[#0B2B26] border border-[#C5A059]/40 rounded-3xl w-full max-w-2xl">
          ...
          <p className="text-gray-300">This weaver profile does not exist or is currently under review.</p>
        </div>
      </div>
    );
  }
